...
# tweets_test = client.search_all_tweets(query=query, tweet_fields=['context_annotations', 'created_at'],
...
#os.chdir("C:\\Users\\Alan Aviles\\Desktop")
#df_eg.to_csv('EG.csv')
#df_100t.to_csv('100T.csv')
#df_tl.to_csv('TL.csv')

# tweet_eg_list = []

# for i in range(1,10):
#         id_batch = df_eg['tweet_id'][(50*(i-1)+1) : 50*i]
#         tweet_eg = [api.get_status(id).text for id in id_batch]
#         tweet_eg_list.append(tweet_eg)
# tweet_eg_list
...
# #get_language(all_tweets_no_atmention[2])

# lang_list = list(map(lambda x: get_language(x), all_tweets_no_atmention)) 


# In[53]:


# lang_df = pd.DataFrame.from_dict(lang_list)
# lang_df['tweet'] = all_tweets_no_atmention
# lang_df.loc[lang_df["language"] =='en' ]
# tweets_en = lang_df['tweet']
